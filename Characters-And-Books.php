	...
	<body>	
		<div>
			<table>
				...
				<tr>
					<td>Discworld Books and Characters</td>		
				</tr>
				<tr>
					<th>BId</th>
					<th>CId</th>
...
if(!($stmt = $mysqli->prepare("SELECT Books_Characters.BId, Books_Characters.CId FROM Books_Characters"))){
...
?>
		...
		<div>
			<form method="post" action="addcharactertobook.php">
			<fieldset>
				<legend>Books and Characters</legend>
				<p>Books</p>
				<select name="Book">
<?php
//prepare statement (mysqli object)
if(!($stmt = $mysqli->prepare("SELECT Books.Id, Books.Title FROM Books"))){
	echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error; //handle error (if not successful display error)
}
//run statement (get back object)
if(!$stmt->execute()){
...
?>
				...
				<p><input type="submit" name="addBC" value="Add Character to Book" /></p>
			</fieldset>
			</form>
		</div>
		<br />
		<div>
			<form method="post" action="filtercharacterbook.php">
				<fieldset>
...
?>
		...
		<br />
		<div>
		<form action=Characters-And-Locations.php>
        <input type="submit" value="Go To Discworld Characters and Locations Table" />
		</div>	
		<br /><br /><br /><br /><br />
	</body>
</html>